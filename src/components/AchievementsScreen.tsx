...
export function AchievementsScreen({ vm }: { vm: ViewModel }) {
  ...
  // it on navigation would clear the NEW state before the user ever saw it.
  useEffect(() => {
    a.markSeen();
    ...
    <div style={{ padding: '24px 20px 100px' }}>
      ...
      <div style={{ background: 'linear-gradient(160deg, oklch(0.22 0.05 35), #17140f 70%)', borderRadius: 20, padding: '20px 18px', marginBottom: 26, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div className="num" style={{ fontSize: 20, fontWeight: 700 }}>{a.tiersEarned}/{a.totalTiers}</div>
          <div style={{ font: "500 11px 'Inter'", color: 'rgba(245,240,234,.5)' }}>tiers earned</div>
        </div>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {cat.items.map((it: any) => (
              <div key={it.id} style={{
                ...
                <div style={{
                  flex: 'none', width: 42, height: 42, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, marginTop: 2,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    <div style={{ font: "600 13px 'Inter'", color: it.unlocked ? '#f5f0ea' : 'rgba(245,240,234,.6)' }}>{it.tierName}</div>
                    <span style={{ font: "600 9px 'Inter'", padding: '2px 7px', borderRadius: 100, background: it.maxed ? 'oklch(0.78 0.15 35 / 0.2)' : 'rgba(255,255,255,.07)', color: it.maxed ? 'oklch(0.82 0.15 35)' : 'rgba(245,240,234,.5)' }}>{it.maxed ? '★ MAX' : it.tierText}</span>
                    {it.isNew && (
                  ...
                  {!it.maxed && (
                    <div style={{ marginTop: 8 }}>
                      <div style={{ height: 4, borderRadius: 3, background: 'rgba(255,255,255,.08)', overflow: 'hidden', marginBottom: 4 }}>
                        <div style={{ height: '100%', width: `${it.progressPct}%`, background: 'oklch(0.65 0.19 35 / 0.7)' }} />
                      </div>
                      <div style={{ font: "500 10px 'Inter'", color: 'rgba(245,240,234,.4)' }}>{it.progressLabel}{it.nextPoints ? ` · +${it.nextPoints} pts` : ''}</div>
                    </div>
                  )}
                </div>
                <div style={{ flex: 'none', textAlign: 'right' }}>
                  <div className="num" style={{ fontSize: 13, fontWeight: 700, color: it.unlocked ? 'oklch(0.78 0.15 35)' : 'rgba(245,240,234,.35)' }}>{it.earnedPoints}</div>
                  <div style={{ font: "500 9px 'Inter'", color: 'rgba(245,240,234,.3)' }}>/ {it.familyPoints}</div>
                </div>
              </div>
            ))}
          </div>
