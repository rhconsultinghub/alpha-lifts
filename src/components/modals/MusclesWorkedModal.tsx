...
export function MusclesWorkedModal({ vm }: { vm: ViewModel }) {
  ...
  return (
    <div onClick={vm.closeBodyModal} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,.6)', zIndex: 30, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#17140f', borderRadius: 22, padding: '18px 18px 20px', width: 260 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', background: 'radial-gradient(ellipse at 50% 40%, rgba(255,255,255,.05), transparent 70%)', borderRadius: 16, padding: '8px 0' }}>
          <BodyDiagram view={vm.bodyView} ranks={vm.currentDay.diagramRanks} width={140} height={259} showStroke />
        </div>
      </div>
    </div>
  );
